...
"""
    segment_set(defs::SegmentDef...) -> NamedTuple

Build an immutable segment registry from a list of SegmentDef values.
Extension is via `merge`: `merge(CORE_SEGMENTS, segment_set(custom_def))`.
"""
function segment_set(defs::SegmentDef...)
    NamedTuple{Tuple(d.name for d in defs)}(defs)
end

"""
    segment_kwargs(segments, name) -> Tuple{Vararg{Symbol}}

Return the keyword arguments accepted by the named segment.
"""
segment_kwargs(segments::NamedTuple, name::Symbol) = getfield(segments, name).kwargs

"""
    all_kwargs(segments) -> Tuple{Vararg{Symbol}}

Return all keyword arguments accepted by any segment in the registry.
"""
function all_kwargs(segments::NamedTuple)
    Tuple(unique(Iterators.flatten(s.kwargs for s in segments)))
end
